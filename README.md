...
3. 点击 Lab 下方的“校验练习”，查看运行时与类型契约测试的结果；也可以运行 `pnpm lesson NN`，例如 `pnpm lesson 01`。先尝试，再看分层提示和折叠答案。
...
维护时保持每课一个 `#lab` 区域，以及 `#solution` 代码块中一个普通命名函数导出；运行工具从此识别 Lab，不另建函数名单。非导出辅助函数不参与自动运行；不要把需要观察的普通案例误写成非导出。
